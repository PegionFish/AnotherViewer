...
import com.hippo.anotherviewer.client.exception.Image509Exception;
...
public final class SpiderQueen implements Runnable {
    ...
    public static final String SPIDER_INFO_FILENAME = ".anotherviewer";
...
//    }
    ...
    public static int findStartPage(@NonNull Context context, @NonNull GalleryInfo galleryInfo) {
        ...
        SiteApplication application = (SiteApplication) context.getApplicationContext();
        msic = SiteApplication.getSpiderInfoCache(application);
...
//                Crashes.trackError(ignore);
    ...
    private static SpiderInfo readSpiderInfoFromCache(@NonNull Context context, long gid) {
        SiteApplication application = (SiteApplication) context.getApplicationContext();
        SimpleDiskCache cache = SiteApplication.getSpiderInfoCache(application);
    ...
    private SpiderInfo readSpiderInfoFromInternet() {
        try {
            ...
            Request request = new SiteRequestBuilder(SiteUrl.getGalleryDetailUrl(
                    mGalleryInfo.gid, mGalleryInfo.token, 0, false), SiteUrl.getReferer()).build();
    ...
    private String getPTokenFromInternet(int index) {
        ...
        try {
            String url = SiteUrl.getGalleryDetailUrl(
    ...
    private class SpiderWorker implements Runnable {
        ...
        private String getPageUrl(long gid, int index, String pToken,
            ...
            if (oldPageUrl != null) {
                ...
                pageUrl = SiteUrl.getPageUrl(gid, index, pToken);
            }
            // Add skipHathKey
            if (skipHathKey != null) {
                if (pageUrl.contains("?")) {
        ...
        private GalleryPageParser.Result fetchPageResultFromHtml(int index, String pageUrl) throws Throwable {
            GalleryPageParser.Result result = SiteEngine.getGalleryPage(null, mHttpClient, pageUrl, mGalleryInfo.gid, mGalleryInfo.token);
        ...
        private GalleryPageApiParser.Result fetchPageResultFromApi(long gid, int index, String pToken, String showKey, String previousPToken) throws Throwable {
            GalleryPageApiParser.Result result = SiteEngine.getGalleryPageApi(null, mHttpClient, gid, index, pToken, showKey, previousPToken);
            if (StringUtils.endsWith(result.imageUrl, URL_509_SUFFIX_ARRAY)) {
                // Get 509
                // Notify listeners
                notifyGet509(index);
        ...
        private boolean downloadImage(long gid, int index, String pToken, String previousPToken, boolean force) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                String referer;
                referer = SiteUrl.getPageUrl(gid, index, pToken);
...
//                    }
                    ...
                    Call call = mHttpImageClient.newBuilder()
                            ...
                            .newCall(new SiteRequestBuilder(targetImageUrl, referer).build());
                    Response response;
                    try {
