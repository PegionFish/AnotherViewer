...
package com.hippo.anotherviewer.ui.scene.download;

import static com.hippo.anotherviewer.spider.SpiderDen.getExistingGalleryDownloadDir;
import static com.hippo.anotherviewer.spider.SpiderDen.getGalleryDownloadDir;
import static com.hippo.anotherviewer.spider.SpiderInfo.getSpiderInfo;
import static com.hippo.anotherviewer.ui.scene.download.part.DownloadAdapter.DRAG_ENABLE;
...
import com.hippo.anotherviewer.callBack.DownloadSearchCallback;
...
import com.hippo.anotherviewer.sync.DownloadSpiderInfoExecutor;
...
//    private final int[] perPageCountChoices = {1, 2, 3, 4, 5};
    ...
    private int mSelectedCategory = SiteUtils.ALL_CATEGORY;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mDownloadManager = SiteApplication.getDownloadManager(context);
    ...
    public void onDestroy() {
        ...
        if (null == manager) {
            Context context = getEHContext();
            if (null != context) {
                manager = SiteApplication.getDownloadManager(context);
            }
        } else {
...
//        mPaginationIndicator.setTotalCount();
    ...
    public View onCreateView3(LayoutInflater inflater,
        ...
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.DOUJINSHI)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.MANGA)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.ARTIST_CG)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.GAME_CG)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.WESTERN)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.NON_H)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.IMAGE_SET)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.COSPLAY)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.ASIAN_PORN)).toUpperCase(Locale.ROOT));
        categoryList.add(Objects.requireNonNull(SiteUtils.getCategory(SiteConfig.MISC)).toUpperCase(Locale.ROOT));
        ...
        mCategorySpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                switch (position) {
                    case 0:
                        selectedCategory = SiteUtils.ALL_CATEGORY;
                        break;
                    case 1:
                        selectedCategory = SiteConfig.DOUJINSHI;
                        break;
                    case 2:
                        selectedCategory = SiteConfig.MANGA;
                        break;
                    case 3:
                        selectedCategory = SiteConfig.ARTIST_CG;
                        break;
                    case 4:
                        selectedCategory = SiteConfig.GAME_CG;
                        break;
                    case 5:
                        selectedCategory = SiteConfig.WESTERN;
                        break;
                    case 6:
                        selectedCategory = SiteConfig.NON_H;
                        break;
                    case 7:
                        selectedCategory = SiteConfig.IMAGE_SET;
                        break;
                    case 8:
                        selectedCategory = SiteConfig.COSPLAY;
                        break;
                    case 9:
                        selectedCategory = SiteConfig.ASIAN_PORN;
                        break;
                    case 10:
                        selectedCategory = SiteConfig.MISC;
                        break;
                    default:
                        selectedCategory = SiteUtils.ALL_CATEGORY;
                        break;
        ...
        if (0 == position) {
            ...
            switch (position) {
                ...
                case 4: {// Move
                    ...
                    List<DownloadLabel> labelRawList = SiteApplication.getDownloadManager(context).getLabelList();
        ...
        new AsyncTask<List<? extends GalleryInfo>, Void, Void>() {
            ...
            protected Void doInBackground(List<? extends GalleryInfo>... params) {
                for (GalleryInfo info : params[0]) {
                    ...
                    SiteDB.removeDownloadDirname(info.gid);
                    if (file != null) {
                        file.delete();
                    }
                }
...
//                SpiderInfo spiderInfo = getSpiderInfo(info);
    ...
    private DownloadInfo createArchiveDownloadInfo(Context context, Uri uri, String fileName) {
        try {
            ...
            downloadInfo.category = SiteUtils.UNKNOWN; // Keep as UNKNOWN, will be handled in display logic
    ...
    private class DeleteDialogHelper implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (checked) {
                ...
                SiteDB.removeDownloadDirname(mGalleryInfo.gid);
                if (file != null) {
    ...
    private class MoveDialogHelper implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            SiteApplication.getDownloadManager(context).changeLabel(mDownloadInfoList, label);
        }
    }
...
//            updateThumbVisibility(false);
    ...
    private void filterByCategory() {
        ...
        if (mSelectedCategory == SiteUtils.ALL_CATEGORY) {
