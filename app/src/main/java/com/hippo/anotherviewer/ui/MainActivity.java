...
import static com.hippo.anotherviewer.util.ClipboardUtil.createAnnouncerFromClipboardUrl;
...
import com.hippo.anotherviewer.callBack.ImageChangeCallBack;
...
import com.hippo.anotherviewer.ui.scene.download.DownloadLabelsScene;
...
import com.hippo.anotherviewer.ui.scene.gallery.list.SubscriptionsScene;
import com.hippo.anotherviewer.ui.scene.sign.GetProfileScene;
import com.hippo.anotherviewer.ui.scene.topList.SiteTopListScene;
import com.hippo.anotherviewer.ui.scene.history.HistoryScene;
import com.hippo.anotherviewer.ui.scene.ProgressScene;
import com.hippo.anotherviewer.ui.scene.gallery.list.QuickSearchScene;
import com.hippo.anotherviewer.ui.scene.SecurityScene;
import com.hippo.anotherviewer.ui.scene.SelectSiteScene;
...
//    private static final String KEY_CLIP_TEXT_HASH_CODE = "clip_text_hash_code";
    ...
    /*---------------
     ...
     ---------------*/
    @Nullable
    private SiteDrawerLayout mDrawerLayout;
    @Nullable
    ...
    private boolean handleIntent(Intent intent) {
        ...
        if (Intent.ACTION_VIEW.equals(action)) {
            ...
            Announcer announcer = SiteUrlOpener.parseUrl(uri.toString());
            if (announcer != null) {
...
//            }
        ...
        mChangeTheme.setOnClickListener(v -> {
            ...
            ((SiteApplication) getApplication()).recreate();
        });
    ...
    private void checkDownloadLocation() {
        ...
        if (uniFile.getUri() != null && "smb".equals(uniFile.getUri().getScheme())) {
            if (new com.hippo.anotherviewer.smb.SmbSettings(this).loadConfig() != null) {
    ...
    private void onInit() {
        ...
        // targetSdk 34 后 Android 13+ 不再有安装时临时授予的通知豁免，需运行时请求（R3-P1）
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            PermissionRequester.request(this, Manifest.permission.POST_NOTIFICATIONS,
                    getString(R.string.notification_rationale), PERMISSION_REQUEST_POST_NOTIFICATIONS);
        }
        SiteCookieStore store = SiteApplication.getSiteCookieStore(getApplicationContext());
        List<Cookie> eCookies = store.getCookies(HttpUrl.get(SiteUrl.HOST_E));
        List<Cookie> exCookies = store.getCookies(HttpUrl.get(SiteUrl.HOST_EX));
        ...
        for (int i = 0, n = cookies.size(); i < n; i++) {
            ...
            switch (cookie.name()) {
                case SiteCookieStore.KEY_IPD_MEMBER_ID:
                    ipbMemberId = cookie.value();
                    break;
                case SiteCookieStore.KEY_IPD_PASS_HASH:
                    ipbPassHash = cookie.value();
                    break;
                case SiteCookieStore.KEY_IGNEOUS:
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.nav_top_lists:
                ...
                nav_top_lists.putString(SiteTopListScene.KEY_ACTION, SiteTopListScene.ACTION_TOP_LIST);
                startSceneFirstly(new Announcer(SiteTopListScene.class)
