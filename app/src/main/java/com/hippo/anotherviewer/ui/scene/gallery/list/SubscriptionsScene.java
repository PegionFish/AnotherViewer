...
public final class SubscriptionsScene extends ToolbarScene {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (ehClient == null){
            this.ehClient = SiteApplication.getSiteClient(context);
        }
    ...
    private class SubscriptionHolder extends AbstractDraggableItemViewHolder implements View.OnClickListener {
        ...
        private void deleteRequest(UserTag userTag){
            String url = SiteUrl.getMyTag();

            if (null == context) {
                return;
            }
            assert userTagList != null;
            SiteClient.Callback callback = new SubscriptionsScene.SubscriptionDetailListener(context, userTagList.stageId, getTag());

            SiteRequest mRequest = new SiteRequest()
                    .setMethod(SiteClient.METHOD_DELETE_WATCHED)
    ...
    private class SubscriptionDetailListener extends SiteCallback<GalleryListScene, UserTagList> {
        ...
        public void onSuccess(UserTagList result) {
            ...
            SiteApplication.saveUserTagList(context,result);
            bindSecond();
        }
