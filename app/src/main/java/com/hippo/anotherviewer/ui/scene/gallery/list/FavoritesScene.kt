...
import com.hippo.anotherviewer.ui.dialog.FavoriteListSortDialog
...
import com.hippo.anotherviewer.widget.JumpDateSelector.OnTimeSelectedListener
...
class FavoritesScene : BaseScene(), EasyRecyclerView.OnItemClickListener,
    ...
    private var mDrawerLayout: SiteDrawerLayout? = null

    @DrawerLifeCircle
    private var mDrawerAdapter: FavDrawerAdapter? = null

    @WholeLifeCircle
    private var mClient: SiteClient? = null
    ...
    override fun onCreateView2(
        ...
        mDrawerLayout = ViewUtils.`$$`(activity, R.id.draw_view) as SiteDrawerLayout
...
//            if ((mHelper.nextHref == null || mHelper.nextHref.isEmpty())&&(mHelper.prevHref == null || mHelper.prevHref.isEmpty())) {
    ...
    private fun onGetFavoritesLocal(keyword: String?, taskId: Int) {
        if (mHelper != null && mHelper!!.isCurrentTask(taskId)) {
            ...
            if (TextUtils.isEmpty(keyword)) {
                list = SiteDB.getAllLocalFavorites()
            } else {
                list = SiteDB.searchLocalFavorites(keyword)
            }
    ...
    private inner class DeleteDialogHelper : DialogInterface.OnClickListener,
        ...
        override fun onClick(dialog: DialogInterface?, which: Int) {
            ...
            if (mUrlBuilder!!.getFavCat() == FavListUrlBuilder.FAV_CAT_LOCAL) { // Delete local fav
                ...
                SiteDB.removeLocalFavorites(gidArray)
                mModifyGiList.clear()
                mHelper!!.refresh()
    ...
    private inner class MoveDialogHelper : DialogInterface.OnClickListener,
        ...
        override fun onClick(dialog: DialogInterface?, which: Int) {
            ...
            if (srcCat == FavListUrlBuilder.FAV_CAT_LOCAL) { // Move from local to cloud
                ...
                SiteDB.putLocalFavorites(mModifyGiList)
                mEnableModify = true
                mModifyFavCat = -1
                mModifyAdd = false
                mHelper!!.refresh()
        ...
        override fun getPageData(taskId: Int, type: Int, page: Int) {
            ...
            if (mEnableModify) {
                ...
                if (mModifyAdd) {
                    ...
                    request.setMethod(SiteClient.METHOD_ADD_FAVORITES_RANGE)
                    request.setCallback(
                    ...
                    val request = SiteRequest()
                    request.setMethod(SiteClient.METHOD_MODIFY_FAVORITES)
                    request.setCallback(
                ...
                val request = SiteRequest()
                request.setMethod(SiteClient.METHOD_GET_FAVORITES)
                request.setCallback(
                    GetFavoritesListener(
                        getContext()!!,
                        activity.getStageId(), getTag(),
...
//                String url = mResult.nextHref;
    ...
    private class AddFavoritesListener(
    ...
    ) : SiteCallback<FavoritesScene?, Void?>(context, stageId, sceneTag) {
        ...
        override fun onFailure(e: Exception?) {
            ...
            SiteDB.putLocalFavorites(mBackup)
    ...
    private class GetFavoritesListener(
        ...
        private val mKeyword: String?
    ) : SiteCallback<FavoritesScene?, FavoritesParser.Result?>(context, stageId, sceneTag) {
    ...
    private inner class RandomFavorite : AsyncTask<Void?, Void?, GalleryInfo?> {
        ...
        override fun doInBackground(vararg v: Void?): GalleryInfo? {
            ...
            if (mUrlBuilder!!.getFavCat() == FavListUrlBuilder.FAV_CAT_LOCAL) {
                ...
                if (TextUtils.isEmpty(keyword)) {
                    gInfoL = SiteDB.getAllLocalFavorites()
                } else {
                    gInfoL = SiteDB.searchLocalFavorites(keyword)
                }
            ...
            try {
                ...
                if ((mHelper!!.lastHref == null || mHelper!!.lastHref.isEmpty()) && (mHelper!!.firstHref == null || mHelper!!.firstHref.isEmpty())) { // only one page
                    ...
                    firstGInfo = SiteEngine.getAllFavorites(
                        mOkHttpClient,
                        mHelper!!.firstHref
                    ...
                    val gInfoL =
                        SiteEngine.getAllFavorites(mOkHttpClient, mHelper!!.lastHref).galleryInfoList
                ...
                val rGInfoL = SiteEngine.getAllFavorites(
                    mOkHttpClient,
