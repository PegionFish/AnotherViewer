...
import com.hippo.anotherviewer.FavouriteStatusRouter;
import com.hippo.anotherviewer.R;
import com.hippo.anotherviewer.Settings;
import com.hippo.anotherviewer.callBack.SubscriptionCallback;
...
import com.hippo.anotherviewer.client.parser.GalleryDetailUrlParser;
...
import com.hippo.anotherviewer.ui.TagSelectorActivity;
import com.hippo.anotherviewer.ui.dialog.SelectItemWithIconAdapter;
...
import com.hippo.anotherviewer.widget.GalleryInfoContentHelper;
import com.hippo.anotherviewer.widget.JumpDateSelector;
import com.hippo.anotherviewer.widget.SearchBar;
import com.hippo.anotherviewer.widget.SearchLayout;
...
public final class GalleryListScene extends BaseScene
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        executorService = SiteApplication.getExecutorService(context);
        mClient = SiteApplication.getSiteClient(context);
        mDownloadManager = SiteApplication.getDownloadManager(context);
        mFavouriteStatusRouter = SiteApplication.getFavouriteStatusRouter(context);
    ...
    private static String getSuitableTitleForUrlBuilder(
        ...
        if (ListUrlBuilder.MODE_NORMAL == urlBuilder.getMode() &&
            ...
            return SiteUtils.getCategory(category);
        } else {
            return null;
        }
    }
    ...
    private ChipGroup buildChipGroup(GalleryInfo gi, ChipGroup tagFlowLayout) {
        ...
        if (null == gi.tgList) {
            ...
            if (Settings.getShowTagTranslations()) {
                if (ehTags == null) {
                    ehTags = SiteTagDatabase.getInstance(getContext());
                }
...
//                String[] parts = text.split("(?=(?:(?:[^\"]*\"){2})*[^\"]*$)\\s+");
            ...
            SiteDB.insertQuickSearch(quickSearch);
    ...
    public boolean onItemLongClick(GalleryInfo gi, View view) {
        ...
        imageViewNew.load(SiteCacheKeyFactory.getThumbKey(gi.gid), gi.thumb);
...
//                .setTitle(SiteUtils.getSuitableTitle(gi))
...
//            if ((mHelper.nextHref == null || mHelper.nextHref.isEmpty())&&(mHelper.prevHref == null || mHelper.prevHref.isEmpty())) {
    ...
    class GalleryListHelper extends GalleryInfoContentHelper {
        ...
        protected void getPageData(int taskId, int type, int page) {
            ...
            if (ListUrlBuilder.MODE_IMAGE_SEARCH == mUrlBuilder.getMode()) {
                ...
                request.setMethod(SiteClient.METHOD_IMAGE_SEARCH);
                ...
                SiteRequest request = new SiteRequest();
                request.setMethod(SiteClient.METHOD_GET_GALLERY_LIST);
                request.setCallback(new GetGalleryListListener(getContext(),
                        activity.getStageId(), getTag(), taskId));
    ...
    private static class GetGalleryListListener extends SiteCallback<GalleryListScene, GalleryListParser.Result> {
    ...
    private static class AddToFavoriteListener extends SiteCallback<GalleryListScene, Void> {
    ...
    private static class RemoveFromFavoriteListener extends SiteCallback<GalleryListScene, Void> {
